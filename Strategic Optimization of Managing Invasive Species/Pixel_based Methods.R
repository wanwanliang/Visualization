...
######## Method 7 --  old infestation potential method  ####
...
Thost2 <- function(ra_ply,host){
  
  host[host>0]=1
  host2 = host
  
  buff_only = ra_ply
  hostN = unlist(lapply(1:length(buff_only), function(x){extract(host2, buff_only[x,], na.rm=T, fun=sum)}))
  hst=as.data.frame(unlist(hostN))
...
host_m6= function(inf,dispersal_rate,reproductive_rate,Nstep,Wcoef,host){
  
  ra_ply=rasterToPolygons(inf,fun=function(x){x>0},dissolve = F)
  ra_ply$ID = 1:length(ra_ply)
  
  buf=range_m6(inf=inf, dispersal_rate=dispersal_rate,reproductive_rate=reproductive_rate,Nstep=Nstep,Wcoef=Wcoef)
  
  buf_size = unlist(lapply(1:length(ra_ply), function(x){extract(buf, ra_ply[x,], fun=mean)}))
  
  ra_ply$buf_size=as.data.frame(buf_size)[,1]
  ply_bf=buffer(ra_ply,width=ra_ply$buf_size,dissolve=F)
  
  buff_only = gDifference(ply_bf, ra_ply,id=as.character(ply_bf$ID),  byid=c(T,F))
  
  buff_only$ID = names(buff_only)
  # calculate number of uninfested host within the buffer for each infested pixel/polygon
  ply_bf2= Thost2(buff_only,host)
  
  ra_ply2 = ra_ply[ra_ply$ID%in%ply_bf2$ID,]
  ra_ply2$Nhost=ply_bf2$Thst
...
treat_m6 = function(inf,dispersal_rate,reproductive_rate,Nstep,Wcoef,host, budget, buffer,cost_per_meter_sq){
  
  host[inf>0]=0
  ra_ply= host_m6(inf,dispersal_rate,reproductive_rate,Nstep,Wcoef,host)
...
treat_m6b = function(inf,dispersal_rate,reproductive_rate,Nstep,Wcoef,ply,host, budget, buffer,cost_per_meter_sq){
  ...
  treatmentRa=rasterize( treatment,inf,field=1,background=0,getCover=T)
  treatmentPly= crop(ra_ply2, treatment)
  #treatmentLs=list(as.matrix(treatmentRa))
  return(treatmentPly)
